...
| Medium — closed | `package.json` | Static dependency analysis flagged unused `@radix-ui/react-label`, `@radix-ui/react-select`, `@radix-ui/react-separator`, and `react-easy-crop`. | Removed in the dead-code cleanup; `@radix-ui/react-slot` retained (used by `components/ui/button.tsx`). |
