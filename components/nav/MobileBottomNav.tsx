...
export function MobileBottomNav() {
  ...
  if (isImmersiveMobilePath(pathname) || isHostPortalPath(pathname)) {
    return null;
  }
  ...
  return (
    <nav
      className="fixed inset-x-0 bottom-0 z-layer-header px-3 pb-[max(0.75rem,env(safe-area-inset-bottom,0px))] pt-3 md:hidden"
      dir={isRtl ? "rtl" : "ltr"}
      aria-label={t("pwa.navAria")}
    >
      {/* Full-bleed opaque chrome so scrolled page content cannot show through gaps around the floating pill. */}
      <div
        aria-hidden
        className="pointer-events-none absolute inset-x-0 bottom-0 top-0 -mx-3 bg-nexa-bg"
      />
      <div
        aria-hidden
        className="pointer-events-none absolute inset-x-0 bottom-full -mx-3 h-8 bg-gradient-to-t from-nexa-bg to-transparent"
      />
      <div className="relative mx-auto max-w-lg">
        <div
          className={cn(
            "flex items-stretch justify-between gap-0.5 rounded-[28px] px-1.5 py-1.5",
            "border border-nexa-line/60 bg-white/95 shadow-[0_8px_40px_rgba(26,17,24,0.14)] backdrop-blur-2xl",
          )}
        >
          {left.map((tab) => {
            const Icon = tab.icon;
