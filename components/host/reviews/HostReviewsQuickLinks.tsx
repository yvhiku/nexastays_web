...
const LINKS = [
  {
    href: "/host/dashboard",
    labelKey: "hostPortal.reviews.linkDashboard",
    icon: Home,
  },
  {
    href: "/host/bookings",
    labelKey: "hostPortal.reviews.linkBookings",
    icon: CalendarDays,
  },
  {
    href: "/host/listings",
    labelKey: "hostPortal.reviews.linkListings",
    icon: Building2,
  },
  {
    href: "/host/analytics",
    labelKey: "hostPortal.reviews.linkInsights",
    icon: BarChart3,
  },
] as const;

export function HostReviewsQuickLinks({ t, localePath }: Props) {
  return (
    <HostPortalCard className="mt-8 p-5 sm:p-6">
      <h2 className="mb-4 flex items-center gap-2 text-lg font-semibold text-[color:var(--host-text)]">
        <Star
          ...
          aria-hidden
        />
        {t("hostPortal.reviews.quickLinksTitle")}
      </h2>
