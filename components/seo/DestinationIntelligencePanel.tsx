...
export function DestinationIntelligencePanel({
  ...
  const perNight = labels.perNight;
  ...
  if (intelligence.avgNightlyPrice != null) {
    rows.push({
      ...
      value: formatNightlyPrice(intelligence.avgNightlyPrice, currency, locale, perNight),
    });
  }
  if (intelligence.minPrice != null) {
    rows.push({
      label: labels.cheapest,
      value: formatNightlyPrice(intelligence.minPrice, currency, locale, perNight),
    });
  }
  ...
  if (intelligence.topNeighborhood) {
    rows.push({ label: labels.topArea, value: cleanText(intelligence.topNeighborhood) });
  }
  if (bestTimeToVisit) {
  ...
  return (
    <section
      ...
      aria-label={`${cleanText(destinationName)} marketplace statistics`}
    >
      <h2 className="font-display text-lg sm:text-xl font-semibold text-nexa-ink mb-5">
        {labels.title.replace("{city}", cleanText(destinationName))}
      </h2>
